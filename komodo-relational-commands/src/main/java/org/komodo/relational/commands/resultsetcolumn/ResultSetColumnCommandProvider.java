 ...
 */
public class ResultSetColumnCommandProvider implements ShellCommandProvider {

    /**
     * Constructs a command provider for Table shell commands.
     */
    public ResultSetColumnCommandProvider() {
        // nothing to do
    }
    ...
    @Override
    public ResultSetColumn resolve ( final Repository.UnitOfWork uow, final KomodoObject kObj ) throws KException {
        if(ResultSetColumn.RESOLVER.resolvable(uow, kObj)) {
            return ResultSetColumn.RESOLVER.resolve(uow, kObj);
        }
        return null;
    }
