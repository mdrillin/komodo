...
/**
 * A shell command provider for Indexes.
 */
public class IndexCommandProvider implements ShellCommandProvider {

    /**
     * Constructs a command provider for Table shell commands.
     */
    public IndexCommandProvider() {
        // nothing to do
    }
    ...
    @Override
    public Index resolve ( final Repository.UnitOfWork uow, final KomodoObject kObj ) throws KException {
        if(Index.RESOLVER.resolvable(uow, kObj)) {
            return Index.RESOLVER.resolve(uow, kObj);
        }
        return null;
    }
