 ...
 */
public class ForeignKeyCommandProvider implements ShellCommandProvider {

    /**
     * Constructs a command provider for Table shell commands.
     */
    public ForeignKeyCommandProvider() {
        // nothing to do
    }
    ...
    @Override
    public ForeignKey resolve ( final Repository.UnitOfWork uow, final KomodoObject kObj ) throws KException {
        if(ForeignKey.RESOLVER.resolvable(uow, kObj)) {
            return ForeignKey.RESOLVER.resolve(uow, kObj);
        }
        return null;
    }
